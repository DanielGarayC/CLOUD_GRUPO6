...
ruta_csv = r"C:\Users\Ricardo\Xd\Documents\CLOUD FINAL\CLOUD_GRUPO6\metrics_2025-11-26 (1).csv"
...
for worker, info in resultado.items():
    ...
    # --- Determinar si supera el umbral según consola ---
    supera = intervalos.get(worker)

    if supera is None:
        supera_umbral = False
        detalles_umbral = "Sin métricas en CSV"
    else:
        # EXACTO como lo quieres:
        supera_umbral = bool(supera)  # 👈 TRUE si supera, FALSE si NO supera
        detalles_umbral = "Supera umbral sostenido" if supera_umbral else "No supera umbral sostenido"

    # --- Formatear motivo del análisis de recursos ---
    razon = "; ".join(info["motivo_si"]) if info["motivo_si"] else ("; ".join(info["motivo_no"]) if info["motivo_no"] else "Sin razones")

    # --- Guardar requerimientos reales ---
    reqs = {
        "cpu": round(info["req_cpu_real"], 2),
        "ram_gb": round(info["req_ram_real"], 2),
        "storage_gb": round(info["req_sto_real"], 2)
    }

    salida.append({
        "zona": zona,
        "worker": worker,
        "puede_desplegar": bool(info["puede_desplegar"]),
        "razon": razon,
        "requerimientos_reales": reqs,
        "supera_umbral": supera_umbral,
        "detalles_umbral": detalles_umbral
    })
